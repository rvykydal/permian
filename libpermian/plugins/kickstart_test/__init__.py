...
@api.events.register_structure('installationUrls')
class InstallationUrlsStructure(OtherStructure):
    pass
...
class KickstartTestWorkflow(GroupedWorkflow):
    ...
    def setup(self):
        ...
        if self.event.installationUrls:
            tree_boot_iso_url, defaults_file_path = self.process_installation_urls(
                self.event.installationUrls
            )
            # If not set explicitly set boot.iso location from installation urls
